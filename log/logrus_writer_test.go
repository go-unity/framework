...
import (
	...
	"github.com/go-unity/framework/contracts/filesystem"
	contractshttp "github.com/go-unity/framework/contracts/http"
	"github.com/go-unity/framework/contracts/validation"
	configmock "github.com/go-unity/framework/mocks/config"
	"github.com/go-unity/framework/support/carbon"
	"github.com/go-unity/framework/support/file"
)

var singleLog = "storage/logs/gounity.log"
var dailyLog = fmt.Sprintf("storage/logs/gounity-%s.log", carbon.Now().ToDateString())
	...
	tests := []struct {
		...
		},
		{
			...
			setup: func() {
				...
				log = NewApplication(mockConfig)
				log.Debug("Debug Gounity")
			},
			assert: func() {
				assert.True(t, file.Contain(singleLog, "test.debug: Debug Gounity"))
				assert.True(t, file.Contain(dailyLog, "test.debug: Debug Gounity"))
			},
		},
		{
			name: "No Debug",
			setup: func() {
				...
				log = NewApplication(mockConfig)
				log.Debug("No Debug Gounity")
			},
			assert: func() {
				assert.False(t, file.Contain(singleLog, "test.debug: No Debug Gounity"))
				assert.False(t, file.Contain(dailyLog, "test.debug: No Debug Gounity"))
			},
		},
		{
			name: "Debugf",
			setup: func() {
				...
				log = NewApplication(mockConfig)
				log.Debugf("Gounity: %s", "World")
			},
			assert: func() {
				assert.True(t, file.Contain(singleLog, "test.debug: Gounity: World"))
				assert.True(t, file.Contain(dailyLog, "test.debug: Gounity: World"))
			},
		},
		{
			name: "Info",
			...
			assert: func() {
				assert.True(t, file.Contain(singleLog, "test.info: Gounity"))
				assert.True(t, file.Contain(dailyLog, "test.info: Gounity"))
			},
		},
		{
			name: "Infof",
			setup: func() {
				...
				log = NewApplication(mockConfig)
				log.Infof("Gounity: %s", "World")
			},
			assert: func() {
				assert.True(t, file.Contain(singleLog, "test.info: Gounity: World"))
				assert.True(t, file.Contain(dailyLog, "test.info: Gounity: World"))
			},
		},
		{
			name: "Warning",
			...
			assert: func() {
				assert.True(t, file.Contain(singleLog, "test.warning: Gounity"))
				assert.True(t, file.Contain(dailyLog, "test.warning: Gounity"))
			},
		},
		{
			name: "Warningf",
			setup: func() {
				...
				log = NewApplication(mockConfig)
				log.Warningf("Gounity: %s", "World")
			},
			assert: func() {
				assert.True(t, file.Contain(singleLog, "test.warning: Gounity: World"))
				assert.True(t, file.Contain(dailyLog, "test.warning: Gounity: World"))
			},
		},
		{
			name: "Error",
			...
			assert: func() {
				assert.True(t, file.Contain(singleLog, "test.error: Gounity"))
				assert.True(t, file.Contain(dailyLog, "test.error: Gounity"))
			},
		},
		{
			name: "Errorf",
			setup: func() {
				...
				log = NewApplication(mockConfig)
				log.Errorf("Gounity: %s", "World")
			},
			assert: func() {
				assert.True(t, file.Contain(singleLog, "test.error: Gounity: World"))
				assert.True(t, file.Contain(dailyLog, "test.error: Gounity: World"))
			},
		},
		{
			name: "Panic",
			...
			assert: func() {
				...
				assert.True(t, file.Contain(singleLog, "test.panic: Gounity"))
				assert.True(t, file.Contain(dailyLog, "test.panic: Gounity"))
			},
		},
		{
			name: "Panicf",
			...
			assert: func() {
				assert.Panics(t, func() {
					log.Panicf("Gounity: %s", "World")
				})
				assert.True(t, file.Contain(singleLog, "test.panic: Gounity: World"))
				assert.True(t, file.Contain(dailyLog, "test.panic: Gounity: World"))
			},
		},
		{
			name: "Code",
		...
		},
		{
			...
			setup: func() {
				...
				log = NewApplication(mockConfig)
				log.Owner("team@gounity.dev").Info("Gounity")
			},
			assert: func() {
				assert.True(t, file.Contain(singleLog, "test.info: Gounity\nowner: \"team@gounity.dev\""))
				assert.True(t, file.Contain(dailyLog, "test.info: Gounity\nowner: \"team@gounity.dev\""))
			},
		},
		{
			...
			setup: func() {
				...
				log = NewApplication(mockConfig)
				log.Request(&TestRequest{}).Info("Gounity")
			},
			assert: func() {
		...
		},
		{
			...
			setup: func() {
				...
				log = NewApplication(mockConfig)
				log.Response(&TestResponse{}).Info("Gounity")
			},
			assert: func() {
				expectedParts := []string{
					`test.info: Gounity`,
		...
		},
		{
			...
			setup: func() {
				...
				log = NewApplication(mockConfig)
				log.User(map[string]any{"name": "kkumar-gcc"}).Info("Gounity")
			},
			assert: func() {
		...
		},
		{
			...
			setup: func() {
				...
				log = NewApplication(mockConfig)
				log.With(map[string]any{"key": "value"}).Info("Gounity")
			},
			assert: func() {
				assert.True(t, file.Contain(singleLog, "test.info: Gounity\ncontext: {\"key\":\"value\"}"))
				assert.True(t, file.Contain(dailyLog, "test.info: Gounity\ncontext: {\"key\":\"value\"}"))
...
func TestLogrus_Fatal(t *testing.T) {
	...
	assert.True(t, file.Contain(singleLog, "test.fatal: Gounity"))
	assert.True(t, file.Contain(dailyLog, "test.fatal: Gounity"))
