...
import (
	...
	"github.com/go-unity/framework/foundation/console"
...
func (app *Application) Path(path string) string {
	return filepath.Join("internal", path)
}
...
func getEnvPath() string {
	...
	if env == support.EnvProduction {
		secretsName := os.Getenv("AWS_SECRESTS_NAME")
		if secretsName != "" {
			envPath = secretsName
		} else {
			color.Redln("Invalid env AWS_SECRESTS_NAME is empty")
			os.Exit(0)
		}
	}
	args := os.Args
