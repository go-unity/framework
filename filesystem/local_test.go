...
func (s *LocalTestSuite) SetupTest() {
	...
	s.mockConfig.On("GetString", "filesystems.disks.local.url").Return("https://gounity.dev").Once()
...
func (s *LocalTestSuite) TestPutFileAs_Image() {
	...
	s.Nil(err)
	s.Equal(filepath.Join("PutFileAs1", "image.jpg"), path)
	s.True(s.local.Exists(path))

	path, err = s.local.PutFileAs("PutFileAs1", fileInfo, "image1.jpg")
	s.Nil(err)
	s.Equal(filepath.Join("PutFileAs1", "image1.jpg"), path)
	s.True(s.local.Exists(path))
...
func (s *LocalTestSuite) TestUrl() {
	s.Equal("https://gounity.dev/Url/1.txt", s.local.Url("Url/1.txt"))

	if env.IsWindows() {
		s.Equal("https://gounity.dev/Url/2.txt", s.local.Url(`Url\2.txt`))
	}
}
